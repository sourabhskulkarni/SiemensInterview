...
> >
> > 🔄 **Related Cross-Question 2:** "Your application uses QR scanning and physical YubiKeys for SSO login. How do you bypass or automate this in a headless CI/CD pipeline using Playwright fixtures and local profiles?"
> >
> > **Answer:** "Since we cannot physically insert a YubiKey or scan a QR code in a headless Linux Docker container, we perform a one-time manual authentication on a secure admin machine to generate the session cookies, localStorage, and token tokens. We save this state into a persistent local browser profile. In our CI/CD pipeline, our custom login Playwright fixture is configured to launch using `browserType.launchPersistentContext(userDataDir)` pointing to this profile. This starts the browser pre-authenticated, completely bypassing the physical QR/YubiKey layer while keeping our tokens securely updated in our credentials manager."
> >
